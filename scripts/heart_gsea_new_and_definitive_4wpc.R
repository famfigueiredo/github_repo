...
gsea_formatting(spleen_res_dnavaccine_vs_conu_4wpc, 'dnavaccine', '4wpc')

gsea_simplified_results_dnavaccine_4wpc <-
  simplify(gsea_results_dnavaccine_4wpc)  # simplifying GO terms to reduce redundancy

nrow(gsea_results_dnavaccine_4wpc)  # 370 GO terms/pathways

nrow(gsea_simplified_results_dnavaccine_4wpc)  # 176 GO terms/pathways
...
spleen_dnavaccine_pathways <- as_tibble(y_dnavaccine) %>%
  arrange(-NES) %>%
  mutate(Count = sapply(strsplit(as.character(core_enrichment), '/'), length)) %>%
...
write_tsv(
  ...
  '~/Documents/PhD/Thesis/quantseq_dataAnalysis/deseq2_dataAnalysis_2024/results/spleen/results_4wpc/pathways/spleen_dnavaccine_pathways.tsv'
)
...
### all genes ###
# gsea formatting starting from a DESeq results table
gsea_formatting(spleen_res_eomes_vs_conu_4wpc, 'eomes', '4wpc')

gsea_simplified_results_eomes_4wpc <-
  simplify(gsea_results_eomes_4wpc)  # simplifying GO terms to reduce redundancy

nrow(gsea_results_eomes_4wpc)  # 409 GO terms/pathways
nrow(gsea_simplified_results_eomes_4wpc)  # 190 GO terms/pathways
...
write_tsv(
  spleen_eomes_pathways,
  '~/Documents/PhD/Thesis/quantseq_dataAnalysis/deseq2_dataAnalysis_2024/results/spleen/results_4wpc/pathways/spleen_eomes_pathways.tsv'
)
...
### all genes ###
# gsea formatting starting from a DESeq results table
gsea_formatting(spleen_res_gata3_vs_conu_4wpc, 'gata3', '4wpc')

gsea_simplified_results_gata3_4wpc <-
  simplify(gsea_results_gata3_4wpc)  # simplifying GO terms to reduce redundancy

nrow(gsea_results_gata3_4wpc)  # 189 GO terms/pathways
nrow(gsea_simplified_results_gata3_4wpc)  # 86 GO terms/pathways
...
write_tsv(
  spleen_gata3_pathways,
  '~/Documents/PhD/Thesis/quantseq_dataAnalysis/deseq2_dataAnalysis_2024/results/spleen/results_4wpc/pathways/spleen_gata3_pathways.tsv'
)
...
### all genes ###
# gsea formatting starting from a DESeq results table
gsea_formatting(spleen_res_ivhd_vs_conu_4wpc, 'ivhd', '4wpc')

nrow(gsea_results_ivhd_4wpc)  # 427 GO terms/pathways
...
nrow(gsea_simplified_results_ivhd_4wpc)  # 203 GO terms/pathways
...
write_tsv(
  spleen_ivhd_pathways,
  '~/Documents/PhD/Thesis/quantseq_dataAnalysis/deseq2_dataAnalysis_2024/results/spleen/results_4wpc/pathways/spleen_ivhd_pathways.tsv'
)
    
# Convert to a Markdown table ----
# Read the TSV file
data <- read.delim('~/Documents/PhD/Thesis/quantseq_dataAnalysis/deseq2_dataAnalysis_2024/results/spleen/results_4wpc/pathways/spleen_ivhd_pathways.tsv', header = TRUE, sep = "\t")
...
### all genes ###
# gsea formatting starting from a DESeq results table
gsea_formatting(spleen_res_ivld_vs_conu_4wpc, 'ivld', '4wpc')

nrow(gsea_results_ivld_4wpc)  # 133 GO terms/pathways

gsea_simplified_results_ivld_4wpc <-
  simplify(gsea_results_ivld_4wpc)

nrow(gsea_simplified_results_ivld_4wpc)  # 96 GO terms/pathways
...
bottom10_low_nes <-
  ...
  filter(NES < 0) %>%
  arrange(desc(setSize)) %>%
  top_n(10, wt = setSize) %>%  # 1 enriched term
  mutate(Count = sapply(strsplit(as.character(core_enrichment), '/'), length))
...
write_tsv(
  spleen_ivld_pathways,
  '~/Documents/PhD/Thesis/quantseq_dataAnalysis/deseq2_dataAnalysis_2024/results/spleen/results_4wpc/pathways/spleen_ivld_pathways.tsv'
)
